...
SKILL_PROFILES = {
    ...
    "caveman": {
        "category": "知识管理",
        "summary": "让 Agent 用极简但保留技术细节的表达方式回复，主要用于减少输出 token 和长线程消耗。",
        "whenToUse": "希望回答更短、更省 token，或明确说“caveman mode”“less tokens”“be brief”时使用。",
        "requirements": "这是一种持续对话风格；需要恢复正常表达时说“stop caveman”或“normal mode”。",
        "examples": ["/caveman", "/caveman ultra", "stop caveman"],
    },
    "caveman-compress": {
        "category": "知识管理",
        "summary": "把 CLAUDE.md、偏好、todo 等自然语言记忆文件压缩成更省 token 的格式，并保留原文件备份。",
        "whenToUse": "需要压缩项目记忆、长期上下文或偏好文件，让后续会话读取更少 token 时使用。",
        "requirements": "会改写目标文件并生成备份；适合自然语言记忆文件，不适合压缩代码。",
        "examples": ["/caveman-compress CLAUDE.md", "/caveman-compress docs/preferences.md"],
    },
    "caveman-help": {
        "category": "知识管理",
        "summary": "显示 caveman 系列命令速查卡，帮助快速了解不同压缩模式和相关 Skill。",
        "whenToUse": "忘记 caveman 命令、模式区别或想查看整套 caveman 工具入口时使用。",
        "requirements": "只显示帮助，不会切换或持久化 caveman 模式。",
        "examples": ["/caveman-help", "caveman help", "what caveman commands"],
    },
    "cavecrew": {
        "category": "代码质量",
        "summary": "判断什么时候把定位代码、轻量修改或 diff review 委托给 caveman 风格子代理，以节省主线程上下文。",
        "whenToUse": "长线程里需要派子代理找代码、做小范围改动、审查 diff，同时想减少返回内容占用时使用。",
        "requirements": "适合明确的小任务；复杂新功能或跨多文件重构仍应在主线程或专门实现流程里处理。",
        "examples": ["use cavecrew to inspect this diff", "delegate to cavecrew investigator", "spawn cavecrew reviewer"],
    },
    "caveman-commit": {
        "category": "代码质量",
        "summary": "生成简洁准确的 Conventional Commit 提交信息，减少空话，强调变更意图和原因。",
        "whenToUse": "需要写 commit message、整理 staged changes 的提交说明，或调用 /caveman-commit 时使用。",
        "requirements": "只生成提交信息，不会自动 stage、commit 或 amend。",
        "examples": ["/caveman-commit", "write a commit message", "generate commit"],
    },
    "caveman-review": {
        "category": "代码质量",
        "summary": "生成高度压缩但可执行的代码审查意见，每条尽量包含位置、问题和修复建议。",
        "whenToUse": "审查 PR、diff 或代码改动，并希望输出更短、更直接的 review comment 时使用。",
        "requirements": "只输出审查意见，不会自动修改代码或批准 PR。",
        "examples": ["/caveman-review", "review this PR", "review the diff"],
    },
    "grill-me": {
        "category": "代码质量",
...
def infer_category(name, description, path="", kind="skill"):
    ...
    rules = [
        ...
        ("代码质量", ["cavecrew", "commit message", "code review", "review diff", "pull request"]),
        ("知识管理", ["caveman", "token", "context", "memory", "compress"]),
